...
## Jsx在书写的时候和html有什么不同?
必须有一个根标签在最外层包裹(因为这里的标签需要babel编译, 编译成`React.createElement(‘标签名’,’属性对象’, ’内容’) )`
双标签和单标签(`<br/>,<hr/>`)都必须闭合, 否则无法通过编译

## 在jsx中如果非要创建多个平行标签, 该如何处理?
在jsx的最外层使用<></>进行包裹;
